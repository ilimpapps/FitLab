...
class FitLabSupabaseUser extends BaseAuthUser {
  FitLabSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> fitLabSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FitLabSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
